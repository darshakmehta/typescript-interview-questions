...
| Feature                 | Syntax Example           | Used For                  |     |
| ----------------------- | ------------------------ | ------------------------- | --- |
| Function Type           | (a: string) => void      | Simple function types     |     |
| Call Signature          | { (a: number): boolean } | Functions with properties |     |
| Construct Signature     | { new (s: string): Obj } | Constructor functions     |     |
| Combined Call/Construct | { (): string; new... }   | Multi-use like Date       |     |
